...
for i = 1:Q
    coff_alpha(i, :) = cofficients_a(M, sam_r, sam_p, r_q(i), phi_q(i), f, R_o);
end
...
coff_alpha = zeros(Q, 2*M+1);
for i = 1:Q
    coff_alpha(i, :) = cofficients_b(M, sam_r, sam_p, r_q(i), phi_q(i), f, R_s, L);
end
...
for i = -60:60
    for j = -60:60
        temp = sound_pressure_outside(D, L, sqrt(x(i+61, j+61)*x(i+61, j+61) + y(i+61, j+61)*y(i+61, j+61)), atan(y(i+61, j+61)/x(i+61, j+61)), f, R_s);
        p(i+61, j+61) = temp;
    end
end
